...
PImage myFace1; // picture of author's face, should be: data/pic.jpg in sketch folder
PImage myFace2; // picture of author's face, should be: data/pic.jpg in sketch folder
void snapPicture() {saveFrame("PICTURES/P"+nf(pictureCounter++,3)+".jpg"); }

// ************************************************************************ COLORS 
color black, white, red, green, blue, yellow, cyan, magenta, darkMagenta; // predefined
...
void defineMyColors() {
  ...
  black=#000000; white=#FFFFFF; // set colors using Menu >  Tools > Color Selector
...
// ************************************************************************ TEXT 
void scribe(String S, int x, int y) {fill(0); text(S,x,y); noFill();} // writes on screen at (x,y) with current fill color
void scribeHeader(String S, int i) {fill(0); text(S,10,20+i*20); noFill();} // writes black at line i
void scribeHeaderRight(String S, int yoffset) {fill(0); text(S,width-7.5*S.length(),yoffset); noFill();} // writes black on screen top, right-aligned
...
void displayTextImage() { // Displays text and authors face on screen
    scribeHeader(title,0); scribeHeaderRight(name1,150); scribeHeaderRight(name2,280); 
